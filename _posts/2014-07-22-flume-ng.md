...
1. 通过interceptors 往 header 里添加 timestamp，这样做，可以在 hdfs.path 引用系统内部的时间变量。
2. 通过设置 hdfs.inUsePrefix，避免在 mr 过程中读到这些临时文件，引起一些错误
...
如果提示找不到 hadoop-test的 jar 包，则修改 pom.xml 中的版本，如改为 `2.0.0-mr1-cdh4.7.0`，具体版本视你使用的分支版本而定，我这里是cdh4.7.0。

如果提示找不到 uanodeset-parser 的 jarb，则在pom.xml 中添加下面仓库：
